...
import { getMapaSala, updateMapaSala } from '../services/mapasala';

function MapaSalaEditPage({ id, onCancel, salaId }) {
  ...
  useEffect(() => {
    if (id) {
      getMapaSala(id).then(data => {
        setForm({ nombre: data.nombre || '', imagen: null });
        setPreview(data.imagenBase64 ? `data:image/*;base64,${data.imagenBase64}` : null);
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      await updateMapaSala(id, { nombre: form.nombre, salaId, imagenBase64 });
      setSuccess(true);
    } catch (err) {
      setError(err);
    } finally {
  ...
  return (
    <Box m={3} maxWidth={500} mx="auto">
      <Paper elevation={3} sx={{ p: 3 }}>
        <Typography variant="h5" mb={2}>Editar Mapa de Sala</Typography>
        <Typography variant="subtitle2" mb={2}>Sala ID: {salaId}</Typography>
        {success && <Alert severity="success" sx={{ mb: 2 }}>¡Mapa actualizado correctamente!</Alert>}
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          ...
          <Button
            ...
            sx={{ mt: 2 }}
          >
            {form.imagen ? form.imagen.name : 'Seleccionar nueva imagen (opcional)'}
            <input
              type="file"
              name="imagen"
              accept="image/*"
...
export default MapaSalaEditPage;
